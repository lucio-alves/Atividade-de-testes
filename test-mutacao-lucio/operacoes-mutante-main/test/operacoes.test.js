...
describe('Suíte de Testes Aprimorada para 50 Operações Aritméticas', () => {
  ...
  test('4. deve dividir dois números e lançar erro específico para divisão por zero', () => {
    expect(divisao(10, 2)).toBe(5);
    expect(() => divisao(5, 0)).toThrow('Divisão por zero não é permitida.');
  });
  test('5. deve calcular a potência com expoente positivo', () => { expect(potencia(2, 3)).toBe(8); });
  test('6. deve calcular a raiz quadrada, tratar zero e lançar erro para negativo', () => { // Teste Atualizado
    expect(raizQuadrada(16)).toBe(4);
    expect(raizQuadrada(0)).toBe(0);
    // NOVO TESTE: Verifica o erro para números negativos
  ...
  test('8. deve calcular o fatorial, tratar casos base e lançar erro para negativo', () => { // Teste Atualizado
  ...
  test('9. deve calcular a média de um array e tratar array vazio', () => {
  ...
  test('11. deve encontrar o valor máximo e tratar array vazio', () => {
    expect(maximoArray([1, 50, 10])).toBe(50);
    expect(() => maximoArray([])).toThrow('Array vazio не possui valor máximo.');
  });
  test('12. deve encontrar o valor mínimo e tratar array vazio', () => {
  ...
  test('31b. deve calcular o MDC com zero', () => {
  ...
  test('33. deve verificar primalidade e tratar casos base', () => {
  ...
  test('35. deve calcular o produto de um array e tratar array vazio', () => {
    expect(produtoArray([2, 3, 4])).toBe(24);
    expect(produtoArray([])).toBe(1);
  });
  test('36. deve manter um valor dentro de um intervalo (clamp) e testar limites', () => {
  ...
  test('37. deve verificar divisibilidade (verdadeiro e falso)', () => {
    expect(isDivisivel(10, 2)).toBe(true);
    expect(isDivisivel(10, 3)).toBe(false);
  });
  test('38. deve converter Celsius para Fahrenheit (zero e não-zero)', () => {
    expect(celsiusParaFahrenheit(0)).toBe(32);
    expect(celsiusParaFahrenheit(100)).toBe(212);
  });
  test('39. deve converter Fahrenheit para Celsius (zero e não-zero)', () => {
    expect(fahrenheitParaCelsius(32)).toBe(0);
    expect(fahrenheitParaCelsius(212)).toBe(100);
  });
  test('40. deve calcular o inverso e tratar zero', () => {
  ...
  test('44. deve verificar maior que (verdadeiro, falso, igual)', () => {
    expect(isMaiorQue(10, 5)).toBe(true);
    expect(isMaiorQue(5, 10)).toBe(false);
    expect(isMaiorQue(5, 5)).toBe(false);
  });
  test('45. deve verificar menor que (verdadeiro, falso, igual)', () => {
    expect(isMenorQue(5, 10)).toBe(true);
    expect(isMenorQue(10, 5)).toBe(false);
    expect(isMenorQue(5, 5)).toBe(false);
  });
  test('46. deve verificar igualdade (verdadeiro e falso)', () => {
    expect(isEqual(7, 7)).toBe(true);
    expect(isEqual(7, 8)).toBe(false);
  });
  test('47. deve calcular a mediana de arrays (ímpar, par, não ordenado)', () => {
    expect(medianaArray([1, 2, 3, 4, 5])).toBe(3);
    expect(medianaArray([3, 1, 4, 5, 2])).toBe(3);
    expect(medianaArray([1, 2, 3, 4])).toBe(2.5);
    expect(medianaArray([4, 1, 3, 2])).toBe(2.5);
    expect(() => medianaArray([])).toThrow('Array vazio не possui mediana.'); // Verifica erro com 'не'
  });
  test('47b. medianaArray não deve modificar o array original', () => {
    const arr = [3, 1, 2];
    medianaArray(arr);
    expect(arr).toEqual([3, 1, 2]);
  });
  test('47c. deve calcular mediana com array não ordenado complexo', () => {
    expect(medianaArray([5, 1, 4, 2, 3])).toBe(3);
  });
  test('47d. deve calcular mediana com números negativos', () => {
    expect(medianaArray([3, -1, 1])).toBe(1);
  });
  test('47e. deve calcular mediana com array par diferente', () => {
    expect(medianaArray([0, 10])).toBe(5);
  });
